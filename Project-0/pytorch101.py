...
def mutate_tensor(
    ...
    for indice in range(len(indices)):
        x[indices[indice]] = values[indice]
...
def count_tensor_elements(x: Tensor) -> int:
    ...
    # Replace "pass" statement with your code
    dim = torch.tensor(x.shape)
    num_elements = torch.prod(dim)
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    range = torch.arange(start, stop+1, dtype=torch.float64)
    x = range[range%10 == 0]
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x[0:4, 0:6] = torch.tensor([[0, 1, 2, 2, 2, 2],
                                [0, 1, 2, 2, 2, 2],
                                [3, 4, 3, 4, 5, 5],
                                [3, 4, 3, 4, 5, 5]])
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    idx = torch.flip(torch.arange(x.shape[0]), dims=(0,))
    y = x[idx]
