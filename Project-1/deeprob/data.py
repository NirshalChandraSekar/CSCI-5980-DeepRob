...
def _extract_tensors(dset, num=None, x_dtype=torch.float32):
    ...
    Input:
    - dset: A deeprob.ProgressObjectsDataset object
...
def progress_objects(num_train=None, num_test=None, x_dtype=torch.float32):
    ...
    dset_train = deeprob.ProgressObjectsDataset(root=".", train=True, download=download)
    dset_test = deeprob.ProgressObjectsDataset(root=".", train=False)
...
def preprocess_progress_objects(
    ...
    if show_examples:
        ...
        plt.imshow(deeprob.tensor_to_image(img))
        plt.axis("off")
        plt.show()
