...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        train_image = torch.flatten(x_train[i])

        for j in range(num_test):
            
            # Flatten the image
            test_image = torch.flatten(x_test[j])

            # For computing the distance find the difference of the elements, square them, add these elements and then take the root of the final sum
            difference = train_image-test_image
            sumation = torch.sum(difference * difference)
            distance = sumation
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    flattened_test = torch.flatten(x_test, start_dim = 1)

    for i in range(num_train):
        train_image = torch.flatten(x_train[i])

        difference = flattened_test - train_image

        sumation = torch.sum(difference*difference, dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    flattened_train = torch.flatten(x_train, start_dim=1)
    flattened_test = torch.flatten(x_test, start_dim=1)


    sum_train = torch.sum(flattened_train*flattened_train, dim=1)
    sum_train = sum_train.view(-1,1)
    sum_test = torch.sum(flattened_test*flattened_test, dim=1)

    dists = (sum_train + sum_test - 2 * torch.mm(flattened_train, flattened_test.T))
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        dist_array = dists[:,i]
        _, indices = torch.topk(dist_array, k, largest=False)
        labels_arr = y_train[indices]
        y_pred[i] = torch.mode(labels_arr).values.view(1,)
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        y_test_pred = predict_labels(dists, self.y_train)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for fold in range(num_folds):
            val_x = x_train_folds[fold]
            val_y = y_train_folds[fold]

            train_x = torch.cat(x_train_folds[:fold] + x_train_folds[fold + 1:])
            train_y = torch.cat(y_train_folds[:fold] + y_train_folds[fold + 1:])

            classifier = KnnClassifier(train_x, train_y)

            accuracy = classifier.check_accuracy(val_x,val_y,k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies:
        check_dict[k] = sum(k_to_accuracies[k]) / len(k_to_accuracies)
    
    best_k = max(check_dict, key = lambda x: check_dict[x])
